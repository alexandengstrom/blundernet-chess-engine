    ...
    def draw_legal_moves(self, moves):
        ...
        for move in moves:
            ...
            for i in range(0, TILE_SIZE, dot_length + gap_length):
                pygame.draw.line(self.window, color, (x + i, y), (min(x + i + dot_length, x + TILE_SIZE), y), 3)

            for i in range(0, TILE_SIZE, dot_length + gap_length):
                pygame.draw.line(self.window, color, (x + i, y + TILE_SIZE - 1), (min(x + i + dot_length, x + TILE_SIZE), y + TILE_SIZE - 1), 3)

            for i in range(0, TILE_SIZE, dot_length + gap_length):
                pygame.draw.line(self.window, color, (x, y + i), (x, min(y + i + dot_length, y + TILE_SIZE)), 3)

            for i in range(0, TILE_SIZE, dot_length + gap_length):
                pygame.draw.line(self.window, color, (x + TILE_SIZE - 1, y + i), (x + TILE_SIZE - 1, min(y + i + dot_length, y + TILE_SIZE)), 3)
